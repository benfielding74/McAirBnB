...
namespace :spaces do
  ...
  task :setup_spaces do
    connection = PG.connect
    connection.exec('CREATE DATABASE spaces;')
  ...
  task :seed_spaces do
    connection = PG.connect :dbname => 'spaces';
    connection.exec ("INSERT INTO spaces (name, description, price) VALUES ('example_name', 'example_description', '9.99');")
  end
  ...
  task :setup_spaces_test do
    connection = PG.connect
    connection.exec('CREATE DATABASE spaces_test;')
